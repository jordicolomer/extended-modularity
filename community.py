...
def modularity(partition, graph) :
    ...
    for com in set(partition.values()) :
        res += (inc.get(com, 0.) / links) - alpha*(deg.get(com, 0.) / (2.*links))**2
    return res
    ...
    while modif  and nb_pass_done != __PASS_MAX :
        ...
        for node in graph.nodes() :
            ...
            for com, dnc in neigh_communities.iteritems() :
                incr =  dnc  - status.degrees.get(com, 0.) * degc_totw*alpha
...
def __modularity(status) :
    ...
    for community in set(status.node2com.values()) :
        ...
        if links > 0 :
            result = result + in_degree / links - ((degree / (2.*links))**2)*alpha
    return result
